...
namespace ProgrammersBlog.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<IDataResult<Category>> Get(int categoryId)
        {
            ...
            if (result != null)
                return new DataResult<Category>(ResultStatus.Success, result);
        ...
        {
            var result = await _unitOfWork.Categories.GetAllAsync(null, c => c.Articles);

            if (result.Count > -1)
                return new DataResult<IList<Category>>(ResultStatus.Success, result);
